...
| `src/server/kiriterm-daemon-client.ts` | runtime-adapter | backend HTTP client for the detached kiriterm session daemon | not-started | required | Added with the kiriterm daemon split; promise-based client behind `TerminalServerApi`; Effect service seam pending. |
| `src/server/kiriterm-daemon.ts` | runtime-adapter | detached terminal session daemon (control API, snapshot persistence) | not-started | required | Added with the kiriterm daemon split; hosts terminal server out-of-process; Effect service seam pending. |
...
| `src/server/read-model-contract.ts` | pure | Versioned read-model index row contract and payload schemas | explicit-non-migration | not-required | Pure schema boundary for Rust/TS indexer exchange; keep Effect-free. |
| `src/server/read-model-indexer.ts` | projection | Native-backed derived read-model index refresh over normalized DB summaries | migrating | required | Rust indexer spike behind TS fallback; review/verification pending. |
...
| `src/server/runtime-acceptance.ts` | pure | Runtime turn acceptance-window helper for control-plane calls | explicit-non-migration | not-required | Pure promise helper; keep Effect-free unless telemetry/resource ownership is added. |
...
| `src/server/terminal-subscriptions.ts` | use-case | wake-mode delivery engine for workflow.await (journaled subscriptions over the terminal registry) | not-started | required | Added with unified await/wake; plain async engine; Effect service seam pending. |
